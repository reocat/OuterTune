    ...
    fun addQueue(
        ...
        if (match != null) { // found an existing queue
            ...
            if (replace) { // force replace
                ...
                if (shuffled) {
                    shuffle(match, false, bypassSaveToDb = true)
            ...
            if (containsAll && match.getSize() == mediaList.size && !forceInsert) { // jump to song, don't add
                ...
                if (anyExts != null) {
                    ...
                    if (shuffled) {
                        shuffle(anyExts, false, bypassSaveToDb = true)
                        match.queuePos = match.queue.indexOf(match.queue.find { it.shuffleIndex == 0 })
                    }
                } else {
                ...
                if (QUEUE_DEBUG)
                    Timber.tag(TAG).d("Adding to queue: extension queue rename + extension queue additive")
            ...
            if (shuffled) {
                shuffle(masterQueues.size - 1, false, bypassSaveToDb = true)
