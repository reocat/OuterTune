...
object YouTube {
    ...
    suspend fun album(browseId: String): Result<AlbumPage> =
        runCatching {
            ...
            if (response.header != null) {
                albumOld(browseId, response)
            } else {
                albumNew(browseId, response)
            }
        }

    private suspend fun albumOld(
        browseId: String,
    ...
    private suspend fun albumNew(
        ...
        val artists =
            header?.straplineTextOne?.runs?.oddElements()?.map {
                Artist(
                    name = it.text,
                    id = it.navigationEndpoint?.browseEndpoint?.browseId,
                )
                    ...
                    ArtistItem(
                        ...
                        playEndpoint =
                            response.contents
                                ...
                                ?.watchEndpoint,
                        shuffleEndpoint =
                            response.header.musicImmersiveHeaderRenderer.playButton
                                ?.buttonRenderer
                                ?.navigationEndpoint
                                ?.watchEndpoint,
                        radioEndpoint =
                            response.header.musicImmersiveHeaderRenderer.startRadioButton
                                ?.buttonRenderer
                                ?.navigationEndpoint
                                ?.watchEndpoint,
                    ),
        ...
        when {
            contents?.gridRenderer != null -> {
                LibraryPage(
                    ...
                    continuation =
                        contents.gridRenderer.continuations
                            ?.firstOrNull()
                ...
                LibraryPage(
                    ...
                    continuation =
                        contents.musicShelfRenderer.continuations
                            ?.firstOrNull()
                            ?.nextContinuationData
                            ?.continuation,
                )
                    ...
                    LibraryContinuationPage(
                        items =
                            contents.gridContinuation.items
                                ...
                                .mapNotNull { LibraryPage.fromMusicTwoRowItemRenderer(it) },
                        continuation =
                            contents.gridContinuation.continuations
                                ?.firstOrNull()
                                ?.nextContinuationData
                                ?.continuation,
                    )
                    ...
                    LibraryContinuationPage(
                        items =
                            contents
                                ...
                                .mapNotNull { LibraryPage.fromMusicResponsiveListItemRenderer(it) },
                        continuation =
                            contents.musicShelfContinuation.continuations
                                ?.firstOrNull()
                                ?.nextContinuationData
                                ?.continuation,
                    )
