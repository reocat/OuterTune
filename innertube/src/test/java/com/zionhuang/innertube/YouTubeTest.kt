...
class YouTubeTest {
    ...
    @Test
    fun `Check 'get_search_suggestion' endpoint`() =
        runBlocking {
    ...
    fun `Check 'next' endpoint`() =
        runBlocking {
            ...
            nextResult =
                youTube.next(WatchEndpoint(videoId = "jF4KKOsoyDs", playlistId = "PLaHh1PiehjvqOXm1J7b2QGy2iAvN84Azb")).getOrThrow()
            assertTrue(nextResult.items.isNotEmpty())
        }

    @Test
    fun `Check 'next' continuation`() =
        runBlocking {
