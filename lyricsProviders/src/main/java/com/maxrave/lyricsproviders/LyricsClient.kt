...
class LyricsClient(
    ...
    suspend fun getLrclibLyrics(
        ...
        val tracksWithLyrics = rs.filter { !it.syncedLyrics.isNullOrEmpty() || !it.plainLyrics.isNullOrEmpty() }

        if (tracksWithLyrics.isEmpty()) {
        ...
        val lrclibObject: LrclibObject? =
            if (duration != null) {
                ...
                tracksWithLyrics.minByOrNull { abs(it.duration.toInt() - durationInSeconds) }
            } else {
                tracksWithLyrics.firstOrNull { !it.syncedLyrics.isNullOrEmpty() } ?: tracksWithLyrics.first()
            }
